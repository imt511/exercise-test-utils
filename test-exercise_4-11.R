...
test_that("Check type and dimensions of `grouped_by_year_and_class` variable", {
  ...
  if(is.data.frame(grouped_by_year_and_class)) {
    if (nrow(grouped_by_year_and_class) == 125){
      if (ncol(grouped_by_year_and_class) == 3) {
        succeed()
      } else {
        msg <- 'Your `grouped_by_year_and_class` data frame does not have the correct number of columns (3)'
        return(fail(msg))
      }
    } else {
...
test_that("Check `grouped_by_year_and_class_wide` variable", {
  expect_has_variable("grouped_by_year_and_class_wide", var_type="list", envir=testEnv)
})

test_that("Check type and dimensions of `grouped_by_year_and_class_wide` variable", {
  grouped_by_year_and_class_wide <- get_var_for_test("grouped_by_year_and_class_wide", testEnv)
  if(is.data.frame(grouped_by_year_and_class_wide)) {
    if (nrow(grouped_by_year_and_class_wide) == 30){
      if (ncol(grouped_by_year_and_class_wide) == 8) {
        succeed()
      } else {
        msg <- 'Your `grouped_by_year_and_class_wide` data frame does not have the correct number of columns (8)'
        return(fail(msg))
      }
    } else {
      msg <- 'Your `grouped_by_year_and_class_wide` data frame does not have the correct number of rows (30)'
      return(fail(msg))
    }
  } else {
    msg <- 'Your `grouped_by_year_and_class_wide` variable is not a data frame'
    return(fail(msg))
  }
})

test_that("Check `grouped_by_year_and_class_long` variable", {
  expect_has_variable("grouped_by_year_and_class_long", var_type="list", envir=testEnv)
})

test_that("Check type and dimensions of `grouped_by_year_and_class_long` variable", {
  grouped_by_year_and_class_long <- get_var_for_test("grouped_by_year_and_class_long", testEnv)
  if(is.data.frame(grouped_by_year_and_class_long)) {
    if (nrow(grouped_by_year_and_class_long) == 125){
      if (ncol(grouped_by_year_and_class_long) == 3) {
        succeed()
      } else {
        msg <- 'Your `grouped_by_year_and_class_long` data frame does not have the correct number of columns (125)'
        return(fail(msg))
      }
    } else {
      msg <- 'Your `grouped_by_year_and_class_long` data frame does not have the correct number of rows (3)'
      return(fail(msg))
    }
  } else {
    msg <- 'Your `grouped_by_year_and_class_long` variable is not a data frame'
    return(fail(msg))
  }
})