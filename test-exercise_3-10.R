...
test_that("Check `unique_cities` variable", {
  expect_has_variable("unique_cities", var_type="character", envir=testEnv)
})

test_that("Check that `unique_cities` only has 1 unique value", {
  unique_cities <- get_var_for_test("unique_cities", testEnv)
  check <- unique_cities
  check <- check[check != ""]
  check <- tolower(check)
  check <- unique(check)
  if (check == 'seattle') {
    ...
    msg <- 'The values of your `unique_cities` vector are incorrect. Are you using the correct column?'
    return(fail(msg))
  }
})
...
test_that("Check type and dimensions of `filtered_permits` variable", {
  ...
  if(is.data.frame(filtered_permits)) {
    if (nrow(filtered_permits) == 8264){
      ...
      msg <- 'Your `filtered_permits` data frame does not have the correct number of rows (8264)'
      return(fail(msg))
    }
  } else {
...
test_that("Check `unique_permit_classes` variable", {
  expect_has_variable("unique_permit_classes", var_type="character", envir=testEnv)
})

test_that("Check length of `unique_permit_classes` variable", {
  unique_permit_classes <- get_var_for_test("unique_permit_classes", testEnv)
  if (length(unique_permit_classes) == 7) {
    succeed()
  } else {
    msg <- 'The length of your `unique_permit_classes` variable is incorrect'
    return(fail(msg))
  }
})

test_that("Check type and dimensions of `test_read_write` variable", {
  test_read_write <- get_var_for_test("test_read_write", testEnv)
  filtered_permits <- get_var_for_test("filtered_permits", testEnv)
  
  if(is.data.frame(test_read_write)) {
    if (identical(dim(filtered_permits), dim(test_read_write))){
      succeed()
      } else {
        msg <- 'Your `test_read_write` data frame does not have the same dimensions as your `filtered_permits` data frame'
        return(fail(msg))
      }
  } else {
    msg <- 'Your `test_read_write` variable is not a data frame'
    return(fail(msg))
  }
})