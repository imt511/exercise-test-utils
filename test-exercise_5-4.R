...
test_that("Check `permits_by_year` variable", {
  expect_has_variable("permits_by_year", var_type="list", envir=testEnv)
})

test_that("Check type and dimensions of `permits_by_year` variable", {
  permits_by_year <- get_var_for_test("permits_by_year", testEnv)
  if(is.data.frame(permits_by_year)) {
    if (nrow(permits_by_year) == 24){
      if (ncol(permits_by_year) == 2) {
        succeed()
      } else {
        msg <- 'Your `permits_by_year` data frame does not have the correct number of columns (2)'
        return(fail(msg))
      }
    } else {
      msg <- 'Your `permits_by_year` data frame does not have the correct number of rows (24)'
      return(fail(msg))
    }
  } else {
    msg <- 'Your `permits_by_year` variable is not a data frame'
    return(fail(msg))
  }
})

test_that("Check `permits_by_year_filtered` variable", {
  expect_has_variable("permits_by_year_filtered", var_type="list", envir=testEnv)
})

test_that("Check type and dimensions of `permits_by_year_filtered` variable", {
  permits_by_year_filtered <- get_var_for_test("permits_by_year_filtered", testEnv)
  if(is.data.frame(permits_by_year_filtered)) {
    if (nrow(permits_by_year_filtered) == 11){
      if (ncol(permits_by_year_filtered) == 2) {
        succeed()
      } else {
        msg <- 'Your `permits_by_year_filtered` data frame does not have the correct number of columns (2)'
        return(fail(msg))
      }
    } else {
      msg <- 'Your `permits_by_year_filtered` data frame does not have the correct number of rows (24)'
      return(fail(msg))
    }
  } else {
    msg <- 'Your `permits_by_year_filtered` variable is not a data frame'
    return(fail(msg))
  }
})
...
test_that("Check `plot2`", {
  plot2 <- get_var_for_test("plot2", testEnv)
  if(is.ggplot(plot2)) {
