...
test_that("Define `height_in_inches`", {
  expect_has_variable("height_in_inches", var_type="double", envir=testEnv)
})

test_that("Check that `height_in_inches` is a whole number", {
  height_in_inches <- get_var_for_test("height_in_inches", testEnv)
  if(is.numeric(height_in_inches) & is.whole(height_in_inches)) {
    succeed()
  } else {
    msg <- sprintf('The value of your `height_in_inches` variable (%4f) must be a positive whole number', height_in_inches)
    return(fail(msg))
  }
})

test_that("Define `height_in_cm`", {
  expect_has_variable("height_in_cm", var_type="double", envir=testEnv)
})

test_that("Correctly calculate `height_in_cm`", {
  height_in_cm <- get_var_for_test("height_in_cm", testEnv)
  height_in_inches <- get_var_for_test("height_in_inches", testEnv)
  
  if(height_in_inches * 2.54 == height_in_cm) {
    succeed()
  } else {
    msg <- sprintf('The value of your `height_in_cm` variable (%4f) is incorrect. Did you use the function correctly?', height_in_cm)
    return(fail(msg))
  }
})

test_that("Define `random_var_1`", {
  expect_has_variable("random_var_1", var_type="double", envir=testEnv)
})

test_that("Define `random_var_2`", {
  expect_has_variable("random_var_2", var_type="double", envir=testEnv)
})

test_that("Define `random_var_seeded_1`", {
  expect_has_variable("random_var_seeded_1", var_type="double", envir=testEnv)
})

test_that("Define `random_var_seeded_2`", {
  expect_has_variable("random_var_seeded_2", var_type="double", envir=testEnv)
})

test_that("Correct values after setting seed part 1", {
  random_var_seeded_1 <- get_var_for_test("random_var_seeded_1", testEnv)
  random_var_seeded_2 <- get_var_for_test("random_var_seeded_2", testEnv)
  set.seed(11)
  random_var_check_1 <- runif(1)
  random_var_check_2 <- runif(1)
  
  if(random_var_check_1 == random_var_seeded_1) {
    if(random_var_check_2 == random_var_seeded_2) {
      succeed()
    } else {
      msg <- sprintf('The value of your `random_var_seeded_1` variable (%f) is incorrect. Did you use the correct seed?', random_var_check_2)
      return(fail(msg))
    }
  } else {
    msg <- sprintf('The value of your `random_var_seeded_2` variable (%f) is incorrect. Did you use the correct seed?', random_var_check_1)
    return(fail(msg))
  }
})

test_that("Define `random_var_seeded_3`", {
  expect_has_variable("random_var_seeded_3", var_type="double", envir=testEnv)
})

test_that("Define `random_var_seeded_4`", {
  expect_has_variable("random_var_seeded_4", var_type="double", envir=testEnv)
})

test_that("Correct values after setting seed part 2", {
  random_var_seeded_3 <- get_var_for_test("random_var_seeded_3", testEnv)
  random_var_seeded_4 <- get_var_for_test("random_var_seeded_4", testEnv)
  set.seed(11)
  random_var_check_3 <- runif(1)
  random_var_check_4 <- runif(1)
  
  if(random_var_check_3 == random_var_seeded_3) {
    if(random_var_check_4 == random_var_seeded_4) {
      succeed()
    } else {
      msg <- sprintf('The value of your `random_var_seeded_3` variable (%f) is incorrect. Did you use the correct seed?', random_var_check_3)
      return(fail(msg))
    }
  } else {
    msg <- sprintf('The value of your `random_var_seeded_4` variable (%f) is incorrect. Did you use the correct seed?', random_var_check_4)
    return(fail(msg))
  }
})